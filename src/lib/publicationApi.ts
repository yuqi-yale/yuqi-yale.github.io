...
class PublicationApi {
    ...
    async getNotes(sortOrder: 'asc' | 'desc' = 'desc', limit?: number) {
        ...
        return notes
            .sort((a, b) => {
                return CompareFunctionLookup[sortOrder](new Date(a.year), new Date(b.year));
            })
            .slice(0, limit);
    }

    async getNotesByTag(tag: string, sortOrder: 'asc' | 'desc' = 'desc', limit?: number) {
        const notes = await publicationApi.getNotes(sortOrder, limit);
        const relatedNotes = notes.filter((post) => post.tagsPub.includes(tag));
    ...
    async getAllTags() {
        ...
        return Array.from(new Set(posts.map((note) => note.tagsPub).flat()));
    }
...
export const publicationApi = new PublicationApi(notion, process.env.PUBLICATION_DATABASE_ID!);
